...
Each successful acquire starts a background renewal goroutine that re-runs the same statement every `-leader-election-renew-interval`, for as long as the caller holds leadership. The moment a renewal attempt stops succeeding — lost to a different holder, or a database error — it cancels the context passed into the running job immediately, rather than waiting for the job to finish on its own: a lease that's lost mid-run needs to stop in-flight work mid-run, not just be noticed at the next natural checkpoint.
...
Each acquired lease also carries a `fence_token`, incremented every time the lease changes hands (not on a same-holder renewal). A downstream consumer that wants to reject writes from a holder that has since lost the lease can compare the token it acquired with against the current one — this package doesn't use the token for anything itself today, but the column exists so that guarantee is available without a schema change later.
