...
func TestSettleConfig_WritesTheBoundCyclesRunUnder(t *testing.T) {
	...
	assert.Equal(t, cfg.Inventory.MetadataStepTimeout+cfg.Inventory.SummaryStepTimeout+wantJobIndex+3*wantStepOverrunAllowance, cfg.Inventory.RunTimeout,
		"the cycle must be widened to cover the steps and their overrun allowance, including the step just widened")
